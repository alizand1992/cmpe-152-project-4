...
public class Scope {
     ...
     */
    public boolean tokenInScope(String id) {
    ...
    public ScopeElement getToken(String id) {
        for (HashSet<ScopeElement> sc : getAllScopes()) {
            for (ScopeElement rhs : sc) {
                if (rhs.getId().equals(id)) {
                    return rhs;
     ...
     */
    public Scope addToken(String id, String type, Object value) throws IllegalArgumentException {
        if (scopes.isEmpty()) {
            createScope();
        }

        if (scopes.peekLast().contains(id)) {
